...
class Model_Payment extends Ci_Model{
    ...
    function get_payments($limit, $offset) {
        $data=$this->dbmodel->select("
            payment_method_id AS id,
            payment_method_name AS name,
            payment_method_datetime AS datetime
        ")
        ->from("mo_info_payment_method")
        ->limit($limit)
        ->offset($offset)
        ->execute();
        return $data;
    ...
    function get_payment_units($id=null,  $offset, $limit){
        $master_data = array();
        $data = $this->dbmodel->select("
            apartment_unit_id,
            apartment_unit_name,
        ...
        ->where('apartment_unit_payment_method_id='. $id . ' AND apartment_unit_type_id=apartment_unit_apartment_unit_type_id '.' AND apartment_certificate_id=apartment_unit_certificate_id')
        ->offset($offset)
        ->limit($limit)
        ->execute();
        $i = 0;
