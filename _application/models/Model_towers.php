...
class Model_towers extends Ci_Model{
   //get all apartment_tower
    function get_towers($limit, $offset) {
        $data=$this->dbmodel->select("
            apartment_tower_id AS id,
            apartment_tower_name AS name,
            apartment_tower_latitude AS latitude,
            apartment_tower_longitude AS longitude,
            apartment_tower_total_room AS total_room,
            apartment_tower_total_floor AS total_floor
        ")
   ...
   //get towers by apartment_tower_id
    ...
    function get_apartmens_units($id=null,  $offset, $limit){
        ...
        ->where('apartment_unit_apartment_tower_id='. $id . ' AND apartment_unit_type_id=apartment_unit_apartment_unit_type_id '.' AND apartment_certificate_id=apartment_unit_certificate_id')
        ->offset($offset)
        ->limit($limit)
        ->execute();
        $i = 0;
